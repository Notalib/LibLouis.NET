...
# SDK stage installs one package, llvm, because packing verifies the binaries it is handed and the
# SDK image ships no object readers - see the comment on that stage.
#
...
# llvm is not here to compile anything: packing verifies each staged binary first, and the SDK image
# carries no object readers at all - not even binutils. verify_native_binary.sh prefers the llvm
# tools anyway, and for the Windows targets it requires them, since binutils cannot read aarch64 PE.
# Retried for the same reason as the toolchain stages above.
RUN set -eu; \
    ok=0; \
    for attempt in 1 2 3; do \
        if apt-get update && apt-get install -y --no-install-recommends llvm; then \
            ok=1; break; \
        fi; \
        echo "apt attempt $attempt failed, retrying" >&2; \
        sleep 10; \
